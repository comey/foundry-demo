## Foundry --- A Better Blockchain Software Tool in Testing
...
The version of Rust that I am using for this tutorial is update on 2023-01-26, rust version 1.67.0 (fc594f156 2023-01-24).
...
foundryup
```
The versions of tools installed with foundryup are as follows:

```sh
foundryup: installed - forge 0.2.0 (b454567 2023-02-06T00:07:54.837614704Z)
foundryup: installed - cast 0.2.0 (b454567 2023-02-06T00:07:54.837614704Z)
foundryup: installed - anvil 0.1.0 (b454567 2023-02-06T00:08:06.996436855Z)
foundryup: installed - chisel 0.1.1 (b454567 2023-02-06T00:08:08.369221674Z)  
```
...
forge init --vscode
```
You can view the meanings of the options and see more options of `forge init` [here](https://book.getfoundry.sh/reference/forge/forge-init).
...
* comparison - `assertLt`, `assertGt`, `assertLe`, `assertGe`
...
If you receive any error when running the command above, please run the following commands and then run the command above again.
...
Open the file `remappings.txt` in the root of the project and add the following configuration:
...
With OpenZeppelin Contracts installed, let's create a new file named `EECE571G22.sol` in the `src` directory and add the following code:
...
contract EECE571G22 is ERC721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("EECE571G22", "571G22") {}
...
In the `test` directory, create a new file named `EECE571G22.t.sol` and add the following code:
...
import "forge-std/Test.sol";
import 'src/EECE571G22.sol';

contract EECE571G22Test is Test {
  EECE571G22 eece571g22;
  address alice = address(0x1);
  address bob = address(0x2);

  function setUp() public {
    eece571g22 = new EECE571G22();
  }

  function testMint() public {
    eece571g22.mint(alice, "testhash");
    address owner_of = eece571g22.ownerOf(0);
...
```sh
forge test --match-contract EECE571G22
```
...
function testTransfer() public {
  eece571g22.mint(alice, "testhash");
  vm.startPrank(alice);
  eece571g22.safeTransferFrom(alice, bob, 0);

  address ownerOf = eece571g22.ownerOf(0);
...
function testBalance() public {
  eece571g22.mint(bob, "testhash");
  eece571g22.mint(bob, "testhash");
  eece571g22.mint(bob, "testhash");

  uint balance = eece571g22.balanceOf(bob);
  assertEq(balance, 3);
}
```
...
To learn more, check out the [Foundry Book](https://book.getfoundry.sh/), [Foundry cheatsheet](https://github.com/dabit3/foundry-cheatsheet).